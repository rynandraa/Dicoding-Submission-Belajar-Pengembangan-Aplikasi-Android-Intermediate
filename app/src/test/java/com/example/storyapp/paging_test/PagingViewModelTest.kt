package com.example.storyapp.paging_test
...
import com.example.storyapp.ui.paging.MainAdapter
import com.example.storyapp.ui.paging.PagingViewModel
import com.example.storyapp.ui.paging.StoryRepository
...
class PagingViewModelTest{
    ...
    fun `when Get Quote Should Not Null and Return Data`() = runTest {
        ...
        val data: PagingData<StoryListResponse> = QuotePagingSource.snapshot(dummyQuote)
        val expectedQuote = MutableLiveData<PagingData<StoryListResponse>>()
        expectedQuote.value = data
        Mockito.`when`(quoteRepository.getStories()).thenReturn(expectedQuote)

        val mainViewModel = PagingViewModel(quoteRepository)
        val actualQuote: PagingData<StoryListResponse> = mainViewModel.story.getOrAwaitValue()
    ...
    fun `when Get Quote Empty Should Return No Data`() = runTest {
        val data: PagingData<StoryListResponse> = PagingData.from(emptyList())
        val expectedQuote = MutableLiveData<PagingData<StoryListResponse>>()
        expectedQuote.value = data
        Mockito.`when`(quoteRepository.getStories()).thenReturn(expectedQuote)
...
class QuotePagingSource : PagingSource<Int, LiveData<List<StoryListResponse>>>() {
    companion object {
